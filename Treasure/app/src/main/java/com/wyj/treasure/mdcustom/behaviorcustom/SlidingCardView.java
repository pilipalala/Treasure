...
import com.wyj.treasure.superadapter.SingleAdapter;
import com.wyj.treasure.superadapter.SuperViewHolder;
...
@CoordinatorLayout.DefaultBehavior(SlidingCardBehavior1.class)
public class SlidingCardView extends FrameLayout {
    ...
    private void init(final Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        SingleAdapter<String> adapter = new SingleAdapter<String>(context, R.layout.adapter_simple_item) {
            @Override
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        if (w != oldw || h != oldh) {
            headerHeight = findViewById(R.id.header).getMeasuredHeight();
        }
    }
